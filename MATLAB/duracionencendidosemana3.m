...
% ----------------------------------------
data = readtable('C:\Users\aleja\OneDrive\Escritorio\TFG\BASESDEDATOS\BASEDATOS_24.03-31.03.xlsx');
...
% Verifica los valores máximos
fprintf('Máximo de corriente: %.3f\n', max(data.Consumo));
fprintf('Mínimo de corriente: %.3f\n', min(data.Consumo));

% Verifica cuántas muestras superan el umbral
fprintf('Muestras sobre umbral (%.1f A): %d\n', umbral_corriente, sum(data.Consumo > umbral_corriente));
...
encendidos_filtrados = encendidos(encendidos < 8);
